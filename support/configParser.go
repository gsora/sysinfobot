...
func CheckConfigFile() (ConfigFile, error) {
	// configuration file is located into $HOME/.config/sysinfobot.json
	userHome, _ := user.Current()
	configFilePath := userHome.HomeDir + "/.config/sysinfobot.json"

	confFileObject, err := os.Open(configFilePath)
	defer confFileObject.Close()
	if err != nil {
		// probably no configuration file is present
		err = errors.New("error: configuration file not found in ~/.config/sysinfobot.json")
	}
